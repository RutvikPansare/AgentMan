...
- **GraphQL Mode:** Toggle button (REST/GQL) in the request editor. GraphQL mode locks method to POST, replaces the Body tab with a Query editor (monospace) + Variables sub-tab (JSON), and adds an Introspect button that fetches the schema via the standard introspection query and lists available query fields. On Send the editor assembles `{ query, variables }` as the request body - the HTTP executor stringifies object bodies and sets `Content-Type: application/json` transparently, so no executor or MCP changes were needed. `mode: 'graphql'` persists on the request in YAML.
- **Visual Polish:** Shared `lib/colors.ts` helpers enforce a consistent Hoppscotch palette across all panels - method badges (GET=green, POST=yellow, PUT=blue, PATCH=orange, DELETE=red) and status badges (2xx green, 3xx blue, 4xx yellow, 5xx red). Replaced per-component inline color switches in CollectionsPanel, HistoryPanel, SpotlightSearch, CapturePanel, App tab bar, and ResponseViewer.
